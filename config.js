...
function forBottomMonitor(widget) {
  const n = Gdk.Display.get_default()?.get_n_monitors() || 1;
  return widget(n - 1); // Assuming the bottom monitor is the last one
}
...
const Windows = () => [
  ...
  forBottomMonitor(BarCornerTopleft),
  forBottomMonitor(BarCornerTopright),
];
...
forBottomMonitor(Bar).catch(print);
