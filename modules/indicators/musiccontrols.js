...
function detectMediaSource(link) {
  ...
  if (domain == "homelab.d") return " Homelab";
  return domain;
}
          ...
          onClicked: async (self) => {
            try {
              ...
              setTimeout(async () => {
                try {
                  const shuffleState =
                    await execAsync("playerctl shuffle").catch(print());
                  const formattedState = shuffleState.trim();
                  shuffleIcon =
                    formattedState === "On" ? "shuffle_on" : "shuffle";
        ...
        Button({
          ...
          onClicked: async (self) => {
            ...
            setTimeout(async () => {
              try {
                const repeatState =
                  await execAsync("playerctl loop").catch(print());
                const formattedState = repeatState.trim();
                repeatIcon =
                  formattedState === "Track"
                    ? "repeat_one_on"
                    : formattedState === "Playlist"
                      ? "repeat_on"
                      : "repeat";
